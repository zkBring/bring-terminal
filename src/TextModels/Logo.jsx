...
// Shader Material with the provided shader
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      float pattern(in vec3 p, inout vec3 q, inout vec3 r) {
          ...
          float f = snoise(p + (1.0 * r));
          return f;
      }

      void main() {
        vec2 st = vUv * iResolution.xy;
        ...
        vec3 color = mix(spectrum[1], spectrum[3], pow(length(q), 4.0));
        color = mix(color, spectrum[0], pow(length(r), 1.4));
        color = mix(color, spectrum[2], f);
    
        gl_FragColor = vec4(color, 1.0); // Assign to built-in gl_FragColor
    }
    `,
  });
};
...
const Banner = ({ position }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (groupRef.current) {
      // Calculate a smooth back-and-forth rotation angle
  ...
  return (
    <group ref={groupRef} position={position}>
      <Text3D ref={textRef} font="/font/sharp.json" size={0.3} height={0.05}>
